...
func ParseJws(raw string) (Jws, error) {
	...
	if len(parts) < 3 {
		return nil, erro.New("lack of JWS parts")
	} else if len(parts) > 3 {
		return nil, erro.New("too many JWS parts")
	}
	return parseJws(parts[0], parts[1], parts[2], raw)
}
...
func parseJws(headPart, clmsPart, sigPart, raw string) (*jws, error) {
	t, err := parseJwt(headPart, clmsPart)
	if err != nil {
		return nil, erro.Wrap(err)
	}

	payld := []byte(raw[:len(headPart)+1+len(clmsPart)])
...
func (this *jws) Sign(keys map[string]crypto.PrivateKey) error {
	...
	if this.payld == nil {
		var err error
		this.payld, err = this.Jwt.Encode()
		if err != nil {
			return erro.Wrap(err)
		}
	}
...
func (this *jws) signEcdsa(key crypto.PrivateKey, hash crypto.Hash) error {
	...
	buff := make([]byte, 64)
	copy(buff[:32], r.Bytes())
	copy(buff[32:], s.Bytes())
	this.sig = buff
	return nil
}
